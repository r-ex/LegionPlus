...
#define MODEL_FILE_ID		(('V'<<24)+('S'<<16)+('D'<<8)+'I')
...
namespace titanfall2
{
	...
	struct studiohdr_t
	{
		...
		vtx::FileHeader_t* GetVTX()
		{
			return reinterpret_cast<vtx::FileHeader_t*>((char*)this + vtxindex);
		}

		vvd::vertexFileHeader_t* GetVVD()
		{
			return reinterpret_cast<vvd::vertexFileHeader_t*>((char*)this + vvdindex);
		}

		vvc::vertexColorFileHeader_t* GetVVC()
		{
			if (flags & STUDIOHDR_FLAGS_USES_UV2 || flags & STUDIOHDR_FLAGS_USES_VERTEX_COLOR)
			{
				return reinterpret_cast<vvc::vertexColorFileHeader_t*>((char*)this + vvcindex);
			}
			else
			{
				return nullptr;
